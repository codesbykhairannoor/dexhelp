...
def run_live_paper_trader():
    ...
    while True:
        try:
            ...
            print("\n" + "-" * 80)
            print(f"[SCAN CYCLE] {time.strftime('%Y-%m-%d %H:%M:%S')} | Mengaudit pasar live (Mode: {dyn_mode})...")
            print("-" * 80)
                ...
                try:
                    ...
                    for addr, pos in list(active_positions.items()):
                        ...
                        if trade_mode == "HOLY_GRAIL_75WR":
                            ...
                            elif price_gain_pct >= (dyn_tp / 2): # Lock half TP
                                sl_price = entry_price * 1.05
                    ...
                    for gem in candidates:
                        ...
                        if security["status"] in ["CLEAN & SAFE", "WARNINGS"]:
                            if score >= min_entry_score:
                                print(f"  [QUANTITATIVE PASS] Score {score} >= {min_entry_score}. Instan approve (MANUAL MODE).")
                                is_approved = True
                            elif not portfolio.get("trade_history") or portfolio["wallet_balance"] == portfolio["initial_capital"]:
                                # FORCE TRADE OVERRIDE: Jika belum pernah trade sama sekali, turunkan standar agar bot jalan
                                if score >= 60:
                                    print(f"  [FORCE TRADE OVERRIDE] Score {score} >= 60. Mengambil risiko karena belum ada trade.")
                                    is_approved = True
